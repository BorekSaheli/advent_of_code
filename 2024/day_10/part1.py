...
for zero in zeros:
    ...
    while current_step is not None:
        ...
        if branches == [[-1, [-1, -1]]]:
            # 9 reached
            trail_order += 1
            nines_loc.add(tuple(current_pos))
            current_step = None
        ...
        if len(branches) > 1:
            # add to queue if more tha one branch exists
            for branch in branches[1:]:
                queue.append(branch)

        if branches:
            next_num, next_pos = branches[0]
            if tuple(next_pos) not in checked:
    ...
    sum_trails += len(nines_loc)

print("--")
print(f"sum trails: {sum_trails}")
